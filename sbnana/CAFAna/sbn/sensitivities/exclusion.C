...
//  TFile fin2("surfaces_numu_clamped.root");
  ...
  std::string suffix2 = "genie_flux_norm";
  std::string suffix3 = "genie_flux";
...
//  surf_nom_fd.DrawContour(crit5sig, 7, kBlue);
  ...
  //surf_syst_nd_fd2.DrawContour(crit5sig, kSolid, kBlue);
  //surf_syst_nd_fd3.DrawContour(crit5sig, kSolid, kMagenta);
...
//  surf_syst_fd.DrawContour(crit5sig, kSolid, kBlack);
  ...
  //lgdis->AddEntry(dummy->Clone(), "XSec + Flux Systs Only");
  //dummy->SetLineColor(kMagenta);
  //lgdis->AddEntry(dummy->Clone(), "MicroBooNE + ICARUS");
  //dummy->SetLineColor(kGreen+3);
  //lgdis->AddEntry(dummy->Clone(), "MicroBoone");
  //dummy->SetLineColor(kBlue);
  //lgdis->AddEntry(dummy->Clone(), "XSec + Flux + Norm Systs");
  dummy->SetLineColor(kBlack);
  lgdis->AddEntry(dummy->Clone(), "Stat + Systs");
