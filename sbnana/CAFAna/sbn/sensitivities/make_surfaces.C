...
void make_surfaces(const std::string anatype = numuStr)
{
  ...
  const auto norm_systs = GetNormSysts();
  GetMECSyst();
...
//  slists["genie_flux"] = systs;
  auto gfn_systs = systs;
  gfn_systs.insert(gfn_systs.end(), norm_systs.begin(), norm_systs.end());
//  slists["genie_flux_norm"] = gfn_systs;
