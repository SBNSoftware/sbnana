...
void debug_plots(const std::string anatype = numuStr)
{
  ...
  //p_nd->DebugPlot(*std::find_if(systs.begin(), systs.end(), [](auto s){return s->ShortName() == "NC"s; }), calc);
  //gPad->Print("nc_debug_plot.pdf(","pdf");
  //p_fd->DebugPlot(*std::find_if(systs.begin(), systs.end(), [](auto s){return s->ShortName() == "NC"s; }), calc);
  //gPad->Print("nc_debug_plot.pdf)","pdf");

  //p_nd->DebugPlot(*std::find_if(systs.begin(), systs.end(), [](auto s){return s->ShortName() == "NCELeta"s; }), calc);
  //gPad->Print("nceleta_debug_plot.pdf(","pdf");
  //p_fd->DebugPlot(*std::find_if(systs.begin(), systs.end(), [](auto s){return s->ShortName() == "NCELeta"s; }), calc);
  //gPad->Print("nceleta_debug_plot.pdf)","pdf");

  for(auto s: systs) {
    p_nd->DebugPlot(s, calc);
    gPad->Print(("debug_plots/"+s->ShortName()+".pdf(").c_str(),"pdf");
    p_fd->DebugPlot(s, calc);
    gPad->Print(("debug_plots/"+s->ShortName()+".pdf)").c_str(),"pdf");
  }
...
//    auto prof = Profile(&fd_nd, calc, syst, 21, -1.05, 1.05, -1, {}, loop_systs);
//    auto prof_fd = Profile(&expt_fd, calc, syst, 21, -1.05, 1.05, -1, {}, loop_systs);
//    auto slice = Profile(&fd_nd, calc, syst, 21, -1.05, 1.05, -1, {}, {});
//    auto slice_fd = Profile(&expt_fd, calc, syst, 21, -1.05, 1.05, -1, {}, {});
//
