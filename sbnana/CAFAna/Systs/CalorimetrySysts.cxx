//Initial work by J. Kim and B. Howard to implement calorimetric shift in CAFAna, figure out how to include the cetlib in here soon
      ...
      for(int i_plane=0; i_plane<3; ++i_plane){
        ...
        for(auto& pt: pfp.trk.calo[i_plane].points){
          ...
          if(kCaloSystMode==CaloSystMode::kGainShift){
            pt.dedx = ( std::exp( (1. + sigma*gain_err) * std::log(alpha + (beta/rho/Efield)*this_dedx) ) - alpha ) / (beta/rho\
/Efield);
          }
