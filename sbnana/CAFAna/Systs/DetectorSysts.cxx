...
namespace ana 
{
    ...
    , fDualSided(fIdx)
  {
    if (!systFile.empty()) { fSystFilePath = systFile; }
      ...
      //fSystFilePath = std::string(sbndata) + "detectorSysts/all_ratios_updates_Jun22.root";
      fSystFilePath = "/exp/icarus/app/users/varanini/Systematics_UpdatedSelection/srcs/sbndata/detectorSysts/all_ratio_containment_allvars_finale_pm1sigma.root";
    }
  }
  ...
  std::vector<const ISyst*> GetDetectorSysts(std::string name_in, Var var_in)
  {
    ...
    */
    ...
    syst_names.insert(std::make_pair("DEDX", 0));

    syst_names.insert(std::make_pair("trigger", 0));
    syst_names.insert(std::make_pair("modshaping", 0));
    syst_names.insert(std::make_pair("stretch", 1));
    syst_names.insert(std::make_pair("noise", 1));
