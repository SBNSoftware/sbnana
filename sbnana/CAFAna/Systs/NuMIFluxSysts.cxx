...
//----------------------------------------------------------------------
void NuMIFluxSyst::Shift(double sigma, caf::SRSliceProxy *slc,
                         double &weight) const {
  ...
  if (slc->truth.baseline < 500) {
    std::cout << "Using NuMIFluxSyst on what appears not to be an icarus file "
                 "(baseline = "
  ...
  if (!fScale[0][0][0]) {
    ...
    if (!sbndata) {
      std::cout << "NuMIFluxSysts: $SBNDATA_DIR environment variable not set. "
                   "Please setup the sbndata product."
                << std::endl;
      abort();
    }
    ...
    const std::string fname =
        std::string(sbndata) +
        "/beamData/NuMIdata/2023-01-24_out_450.37_7991.98_79512.66_qel7.root";
    TFile f(fname.c_str());
    if (f.IsZombie()) {
    ...
    for (int hcIdx : {0, 1}) {
      for (int flavIdx : {0, 1}) {
        for (int signIdx : {0, 1}) {
          ...
          if (!h) {
            std::cout << "NuMIFluxSysts: failed to find " << hName << " in "
                      << f.GetName() << std::endl;
            abort();
          }
  ...
  const int hcIdx = 0; // TODO always FHC
...
const NuMIFluxSyst *GetNuMIFluxSyst(const std::string &dir,
  ...
  if (cache.count(key) == 0)
    cache[key] = new NuMIFluxSyst(dir, prefix, name);
...
std::vector<const ISyst *> GetNuMIHadronProductionFluxSysts() {
  const std::vector<std::string> syst_names = {
      "pCpi", "pCk",    "pCnu",        "nCpi",  "mesinc",
      "nua",  "nuAlFe", "attenuation", "others"};

  std::vector<const ISyst *> ret;
  for (std::string name : syst_names)
    ret.push_back(GetNuMIFluxSyst("fractional_uncertainties/hadron/" + name,
                                  "hfrac_hadron_", name));
  return ret;
}
...
std::vector<const ISyst *> GetNuMIBeamlineFluxSysts() {
  const std::vector<std::string> syst_names = {
      "horn_current_plus", "horn1_x",   "horn1_y",     "beam_shift_minus_y",
      "beam_shift_plus_y", "beam_spot", "water_layer", "beam_div"};

  std::vector<const ISyst *> ret;
  for (std::string name : syst_names)
    ret.push_back(GetNuMIFluxSyst("fractional_uncertainties/beam/" + name,
                                  "hfrac_beam_", name));
  return ret;
}
...
std::vector<const ISyst *> GetNuMIPCAFluxSysts(unsigned int Npcs) {
  ...
  for (unsigned int i = 0; i < Npcs; ++i) {
    ret.push_back(
        GetNuMIFluxSyst("pca/principal_components", "hpc_", std::to_string(i)));
  }
  return ret;
}
