...
alp_Gray_x = np.concatenate((alp_Gray_x, [alp_Gray_x[0]])) # close off the contour
alp_Gray_y = np.concatenate((alp_Gray_y, [alp_Gray_y[-1]]))  # close off the contour
...
# note: I used Untitled.ipynb as a scratch pad to figure this out.

charm_global_upper_bound_in_oneOverFa_x = charmP0_gg_x_1
charm_global_upper_bound_in_oneOverFa_y = charmP0_gg_y_1

orange_lb_interp = interp1d(charmP0_uu_x_1, charmP0_uu_y_1)
red_lb_interp = interp1d(charmP0_gg_x_2, charmP0_gg_y_2)
def diff_func(x):
    orange = orange_lb_interp(x)
    red = red_lb_interp(x)
    return orange-red
initial_guess = 0.39
x_intersection = fsolve(diff_func, initial_guess)[0]

mask_1 = (charmP0_uu_x_1 < x_intersection)
global_lb_x_1 = charmP0_uu_x_1[mask_1]
global_lb_y_1 = charmP0_uu_y_1[mask_1]

mask_2 = (charmP0_gg_x_2 > x_intersection)
global_lb_x_2 = charmP0_gg_x_2[mask_2]
global_lb_y_2 = charmP0_gg_y_2[mask_2]

charm_global_lower_bound_in_oneOverFa_x = np.concatenate((global_lb_x_1, [x_intersection], global_lb_x_2))
charm_global_lower_bound_in_oneOverFa_y = np.concatenate((global_lb_y_1, [orange_lb_interp(x_intersection)], global_lb_y_2))
