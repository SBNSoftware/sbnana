...
# ------------------------------------------------------------------------
# Main function:
def pushEventSel(evtdf, selection_key, CL=0.9, pc_uncertainty_sig=0.3, pc_uncertainty_bg=0.6, make_contours=True):#, obs_data=2):
    ...
    # ALP SENSITIVITY (assuming running coupling between cmu and fa.)
        ...
        try:
            ...
            plt.contourf(X_alp_fromP0, 1./np.array(Y_alp), np.log(Z_mixProd_fromP0), levels=[np.log(exp_limits[0]*mc_signal),np.log(exp_limits[1]*mc_signal)],colors='pink') # Test 251114. I expect this to be on the outer edge of the contour.
            ...
            # general formatting
            #plt.title('Selection '+selection_name+'\n%a%% CL \nassuming $\\delta_{sig}$=%a%%, $\\delta_{bg}$=%a%%' % (CL*100, pc_uncertainty_sig*100, pc_uncertainty_bg*100) )
            plt.title('(WORK IN PROGRESS) \n %a%% CL projected sensitivity for 2.41x10$^{20}$ POT, \nassuming $\\delta_{sig}$=%a%%, $\\delta_{bg}$=%a%%' % (CL*100, pc_uncertainty_sig*100, pc_uncertainty_bg*100) ) #250919
            plt.yscale('log')
            plt.xlabel('$m_a$ (GeV)')
            plt.ylabel('$1/f_a$')
            plt.xlim((0.22, 0.45))
            ...
            plt.savefig(output_path + alp_plotname, format='png', bbox_inches='tight', dpi=350)
            plt.close()
...
if __name__ == "__main__":
    for key in selection_keys:
        if len(sys.argv) > 2:
            ...
            pushEventSel(evtdf, key, CL=CL, pc_uncertainty_sig=pc_uncertainty_sig, pc_uncertainty_bg=pc_uncertainty_bg, make_contours=True)
        else:
